...
[diffusionpi](https://github.com/osolmaz/diffusionpi) repository. Install it
into plain Pi from a local checkout (Pi git installs point at repository
roots, so clone first):

```bash
git clone https://github.com/osolmaz/diffusionpi
pi install ./diffusionpi/packages/diffusion-canvas
```

Or launch the bundled app with `bin/diffusionpi`, which loads this package
into a stock Pi session via pi-factory.
