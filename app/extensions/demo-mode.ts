// Self-driving demo for DiffusionPi: sends a story prompt on startup, keeps
// asking the model to continue after every turn, and strips the TUI down to
// the chat plus a single status line. Everything is gated on
// DIFFUSIONPI_DEMO=1 so the same extension can stay loaded in interactive
// sessions without doing anything.
...
const demoEnabled = process.env["DIFFUSIONPI_DEMO"] === "1";
...
async function promptText(file: string, fallback: string): Promise<string> {
  try {
    const promptsDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../prompts");
    const text = (await readFile(path.join(promptsDir, file), "utf8")).trim();
...
export default function diffusionpiDemoMode(pi: ExtensionAPI): void {
  if (!demoEnabled) {
    return;
  }
  let started = false;
  ...
  function queueInitialPrompt(): void {
    void promptText("demo-initial.md", fallbackInitialPrompt).then((prompt) => {
      if (!stopped) {
  ...
  function queueFollowup(): void {
    void promptText("demo-followup.md", fallbackFollowupPrompt).then((prompt) => {
